...
const API_ROOT = "http://api.goodlearn-dz.com/v1";
...
const Car = {
  ...
  attachCarToUser: (user_id, car_id) =>
    requests.post("/cars/attache-car-to-user", { user_id, car_id }),
...
const Tracker = {
  ...
  ping: (id, position) =>
    requests.get("/trackers/" + id + "/ping", { params: position })
};

const Key = {
  getKey: id => requests.get("/keys/" + id),
  all: () => requests.get("/keys"),
  create: key => requests.post("/keys", { ...key }),
  delete: id => requests.delete("/keys/" + id),
  update: key => requests.put("/keys/" + key.id, { ...key }),
};
